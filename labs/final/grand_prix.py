...
def parkInElevator():
    global speed, angle, curState, contour_center, elevator_end
    ...
    if len(markers) > 0:
        ...
        ar_marker.detect_colors(color_image, potential_colors)

        if marker_distance > 155 or ar_marker.get_color() == "BLUE":
            speed = 1
        elif marker_distance < 155 or ar_marker.get_color() == "RED" or ar_marker.get_color == "ORANGE":
            speed = 0
    elif len(markers) == 0 and contour_center is not None:
        elevator_end = True
